...
public class _2TypeSelector implements ContextSelector {
    ...
    public Context selectContext(CSCallSite callSite, CSObj recv, JMethod callee) {
        // TODO - finish me
        if(recv.getContext().getLength() < 2){
            if(recv.getContext().getLength() == 0)
                return ListContext.make(recv.getObject().getContainerType());
            else{
                Type type = (Type)recv.getContext().getElementAt(0);
                return ListContext.make(type, recv.getObject().getContainerType());
            }
        }
        Type type = (Type)recv.getContext().getElementAt(1);
        return ListContext.make(type, recv.getObject().getContainerType());
    }
