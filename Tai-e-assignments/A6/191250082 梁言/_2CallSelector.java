...
public class _2CallSelector implements ContextSelector {
    ...
    public Context selectContext(CSCallSite callSite, JMethod callee) {
        ...
        //检查一下caller上下文有几个元素
        //<2说明没有满，直接往里面加就可以了
        if(callSite.getContext().getLength() < 2){
            if(callSite.getContext().getLength() == 0)
                return ListContext.make(callSite.getCallSite());
            else{
                Invoke invoke = (Invoke) callSite.getContext().getElementAt(0);
                return ListContext.make(invoke, callSite.getCallSite());
            }
        }
        //==2说明两个元素
        Invoke invoke = (Invoke) callSite.getContext().getElementAt(1);
        return ListContext.make(invoke, callSite.getCallSite());
    }
